...
# Installation
To install storm using the [provision-storm.yml](provision-storm.yml) playbook in this repository, first clone the contents of this repository to a local directory using a command like the following:

```bash
$ git clone https://github.com/Datanexus/storm
```

That command will pull down the repository, which includes the [provision-storm.yml](provision-storm.yml) playbook and all of its dependencies.
...
* It avoids modifying files that are being tracked under version control in the main, [storm](https://github.com/Datanexus/storm) repository (the first option); making such changes will, more than likely, lead to conflicts at a later date when these files are modified in the main [storm](https://github.com/Datanexus/storm) repository in a way that is inconsistent with the values that you have set locally (in your clone of this repository).
...
That being said, the second option may be useful for some deployment scenarios (a one-off deployment of a local test environment, for example), so it remains a viable option for some users. Overall, we would recommend against trying to maintain your preferred ensemble configuration using the values defined in the [vars/storm.yml](vars/storm.yml) file or the default configuration file (the [config.yml](config.yml) file).

# Assumptions
It is assumed that this playbook will be run on a recent (systemd-based) version of RHEL or CentOS (RHEL-7.x or CentOS-7.x, for example); no support is provided for other distributions (and the `site.xml` playbook will not run successfully). Furthermore, it is assumed that you are interested in deploying a relatively recent version of Apache Storm using this playbook (the current default for this playbook is v1.1.1, but any recent v1.x release should do).
